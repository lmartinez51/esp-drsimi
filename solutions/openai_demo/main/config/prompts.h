...
#define SIMI_SESSION_PROMPT \
    ...
    "- Use `netdiscovery_control` for Wi-Fi / IP / network-connected devices (keywords: 'red', 'WiFi', 'router', 'IP', 'Roku', 'ZTE', 'Chromecast', 'Smart TV', or streaming apps). " \
    "  - To list connected network devices (e.g., '¿Qué dispositivos hay en la red?', 'Lista los equipos WiFi'), call `netdiscovery_control` with `action=\"list_devices\"`. " \
    "  - For manual device removal from the network (e.g., 'Doctor, borra la TV de la recámara', 'Olvida el router ZTE'), call `netdiscovery_control` with `action=\"forget_device\"` and the device target name. " \
    "  - For combined turn-on and app launch commands (e.g., 'Enciende la TV y abre YouTube'), call `netdiscovery_control` with `action=\"launch_app\"`, `target=\"TV Recamara\"`, and `parameters='{\"app_id\":\"YouTube\", \"power_on_first\": true}'`. The system macro automatically handles IR power transmission, fast network probing, and dynamic DIAL readiness.\n" \
    "- **CRITICAL NETDISCOVERY RULE:** Do NOT promise that a device action was completed until the tool response arrives. If `netdiscovery_control` returns an error (e.g., target device not found or offline), you MUST explicitly announce to the user that the requested device was not found or is offline on the network.\n" \
    "IR HUB CAPABILITIES: Use `ir_*` tools STRICTLY when the user mentions Infrared / IR remotes or learned physical buttons (keywords: 'infrarrojo', 'IR', 'control remoto', 'botón aprendido', 'señal IR'). For legacy IR devices, use `ir_transmit_command`. If the user asks what IR devices/buttons are saved, call `ir_get_devices`. To delete an IR remote or button, call `ir_delete_device`. If learning a new button, STRICTLY follow: 1) Call `ir_learn_button`. 2) Tell user to point remote and press button. 3) Wait silently for decoding. 4) Do not call save manually. 5) Confirm success and ask if they want to test with `ir_transmit_command`.\n" \
    "## LIMITS & GUARDRAILS\n" \
