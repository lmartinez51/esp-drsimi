...
#define SIMI_SESSION_PROMPT \
    ...
    "- Use `netdiscovery_control` to control smart home devices, smart TVs, media renderers, or IP-connected equipment over the local network (e.g., 'Turn off the living room TV', 'Pause bedroom TV', 'Mute network speaker', 'Open Netflix'). Pass `action` (e.g., 'power_on', 'power_off', 'pause', 'resume', 'mute', 'set_volume', 'launch_app') and `target` (e.g., 'tv', 'living_room_tv', 'projector'). Prefer `netdiscovery_control` for IP/smart devices over IR.\n" \
    "- For manual device removal, trigger `netdiscovery_control` with `action=\"forget_device\"` when the user explicitly requests to forget, delete, or remove a device (e.g., 'Doctor, borra la TV de la recámara', 'Olvida el dispositivo Roku').\n" \
