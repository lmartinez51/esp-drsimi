...
public:
    ...
    static void StartPinnedThread(const char* name, int stackSize, int priority, int coreId, Function f) {
    ...
    template<typename Function>
    static void StartInternalPinnedThread(const char* name, int stackSize, int priority, int coreId, Function f) {
        auto* funcPtr = new Function(std::move(f));
