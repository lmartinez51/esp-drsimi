...
std::optional<std::string> NvsSettingsRepository::GetString(const std::string& key) {
    ...
    nvs_handle_t handle;
    if (nvs_open(m_namespace.c_str(), NVS_READONLY, &handle) != ESP_OK) return std::nullopt;
...
bool NvsSettingsRepository::SetString(const std::string& key, const std::string& value) {
    std::lock_guard<std::mutex> lock(m_mutex);
    nvs_handle_t handle;
    if (nvs_open(m_namespace.c_str(), NVS_READWRITE, &handle) != ESP_OK) return false;
...
std::optional<bool> NvsSettingsRepository::GetBool(const std::string& key) {
    auto res = GetInt(key);
    if (res.has_value()) return (res.value() != 0);
    return std::nullopt;
}
